...
void PowNode::backwardProp(bool first) {
	...
	double new_grad_1 = in_1->get_grad() + m_grad * pow(in_1->get_val(), p - 1) * p; // d/da(a^b) = b * a^(b-1) // Power rule
	double new_grad_2 = in_1->get_grad() + m_grad * (m_val * ln(in_1->get_val())); // d/db(a^b) = a^b * ln(a) // Exponantiation derivative
