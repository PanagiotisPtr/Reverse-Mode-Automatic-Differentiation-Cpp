...
void LogNode::forwardProp() { m_val = log(in_2->get_val()) / log(in_1->get_val()); } // log(base, x) = log(x)/log(base)

void LogNode::backwardProp(bool first) {
	if (first)m_grad = 1;
	double p = in_2->get_val();
	double new_grad_1 = in_1->get_grad() + m_grad * (-1) * ln(in_2->get_val()) / (ln(in_1->get_val())*ln(in_1->get_val())*in_1->get_val()); // d/dbase(log(base, x)) = -ln(x)/(ln(base)*ln(base)*base)
	double new_grad_2 = in_2->get_grad() + m_grad * 1 / (in_2->get_val() * ln(in_1->get_val())); // d/dx(log(base, x)) = 1/(x * ln(base))
